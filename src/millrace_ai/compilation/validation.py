...
class _ResolvedRepairClosureBinding:
    ...
    *,
    ...
    for policy in workflow_primitives.runtime_failure_policies:
        ...
        if runtime_effect_policy and policy.action == "route_to_node":
            _validate_runtime_effect_repair_route_closure(
                policy,
                active_planes=active_planes,
                artifact_contracts_by_id=artifact_contracts_by_id,
                families_by_id=families_by_id,
...
def _validate_runtime_effect_repair_route_closure(
    ...
    *,
    ...
    bindings = _resolve_route_to_node_repair_closures(
        policy,
        runtime_effect_operations_by_id=runtime_effect_operations_by_id,
        runtime_effect_runners_by_id=runtime_effect_runners_by_id,
    )
...
def _resolve_route_to_node_repair_closures(
    policy: RuntimeFailurePolicyDefinition,
    *,
    runtime_effect_operations_by_id: dict[str, RuntimeEffectOperationDefinition],
    runtime_effect_runners_by_id: dict[str, RuntimeEffectOperationRunnerDefinition],
) -> tuple[_ResolvedRepairClosureBinding, ...]:
    ...
    resolved: list[_ResolvedRepairClosureBinding] = []
...
def _validate_repair_route_family_scope(
    policy: RuntimeFailurePolicyDefinition,
    *,
    bindings: tuple[_ResolvedRepairClosureBinding, ...],
) -> None:
...
def _route_policy_operation_ids(
    ...
    *,
    ...
    inferred_operation_ids = {
        operation_id
        for handler_id in policy.applies_to_handler_ids
        for operation_id in _operation_ids_for_legacy_handler(
            handler_id,
            runtime_effect_operations_by_id=runtime_effect_operations_by_id,
            runtime_effect_runners_by_id=runtime_effect_runners_by_id,
        )
...
) -> _ResolvedRepairClosureBinding:
    return _ResolvedRepairClosureBinding(
...
def _validate_explicit_repair_mapping_matches_contract(
    *,
    policy_id: str,
    mapping: _ResolvedRepairClosureBinding,
    contract: _ResolvedRepairClosureBinding,
) -> None:
    for field_name in (
        "repair_operation_id",
        "target_node_id",
...
def _validate_route_target_can_claim_family(
    policy: RuntimeFailurePolicyDefinition,
    *,
    binding: _ResolvedRepairClosureBinding,
