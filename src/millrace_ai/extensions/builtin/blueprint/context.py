...
from millrace_ai.runtime.context.generic import visible_artifact_refs
from millrace_ai.runtime.context.models import RequestContextAuthority, RequestContextRenderPlan
...
def built_in_request_context_provider_registrations() -> tuple[tuple[str, object], ...]:
    return (
...
def built_in_blueprint_provider_registrations() -> tuple[tuple[str, object], ...]:
    return built_in_request_context_provider_registrations()
...
def _artifact_contracts_for_request(
    ...
    *,
    ...
    if compiled_plan is None:
        raise ValueError("compiled plan is required for artifact contract authority")
    contracts_by_id = compiled_plan.artifact_contracts_by_id
    if not contracts_by_id:
        raise ValueError(
            f"compiled plan {compiled_plan.compiled_plan_id} is missing artifact contract authority"
        )
