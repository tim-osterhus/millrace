...
REQUIRED_RUNTIME_DOC_MARKERS = {
    ...
    "docs/RUNTIME_DEEP_DIVE.md": (
        "stable runtime deep-dive portal and compatibility entrypoint",
        "docs/runtime/README.md",
        "docs/TUI_DOCUMENTATION.md",
        "## 1. Boundary Map",
        ...
        "README.md` and `OPERATOR_GUIDE.md` should link here",
        "tests/test_package_parity.py",
        "tests/test_baseline_assets.py",
        "## 4. Docs-Proof Commands",
        "packaged_docs_and_operator_assets_exist",
        "packaged_runtime_docs_reflect_current_resolver_behavior",
    ...
    "docs/runtime/control-plane-command-surface-and-mailbox-semantics.md": (
        ...
        "## 2. Source-Of-Truth Surfaces",
        "`millrace_engine/cli.py`",
        "`millrace_engine/control.py`",
        "`millrace_engine/control_actions.py`",
        "`millrace_engine/control_runtime_surface.py`",
        "`millrace_engine/engine_mailbox_command_handlers.py`",
        "`active_task clear` while the daemon is running is deferred until a daemon boundary",
        "`active_task recover` while the daemon is running is blocked, not deferred",
    ...
    "docs/runtime/stage-execution-pipeline-and-plane-handoff-contracts.md": (
        ...
        "## 2. Source-Of-Truth Surfaces",
        "`millrace_engine/planes/execution.py`",
        "`millrace_engine/planes/execution_runtime.py`",
        "`millrace_engine/stages/base.py`",
        "`millrace_engine/contract_runtime.py`",
        "`ExecutionResearchHandoff`",
        "`CrossPlaneParentRun`",
        "`transition_history.jsonl`",
        "`StageExecutionError`",
        "tests/test_execution_plane.py",
    ),
    "docs/runtime/runner-adapters-model-selection-and-permission-profile-semantics.md": (
        ...
        "## 2. Source-Of-Truth Surfaces",
        "`millrace_engine/config_runtime.py`",
        "`millrace_engine/config.py`",
        "`millrace_engine/contract_runtime.py`",
        "`millrace_engine/stages/base.py`",
        "`millrace_engine/runner.py`",
        ...
        "`allow_network` is exported and carried through `StageContext`, but `CodexRunner.build_command()` does not currently translate it into a Codex CLI flag.",
    ...
    "docs/runtime/configuration-surfaces-apply-boundaries-and-live-reload-semantics.md": (
        ...
        "`cycle_boundary`",
        "`startup_only`",
        "`pending_config_hash` and `pending_config_boundary`",
        "tests/test_config.py",
        "tests/test_cli.py",
    ),
    "docs/runtime/observability-reports-tui-and-audit-truth-surfaces.md": (
        ...
        "## 2. Source-Of-Truth Surfaces",
        "`millrace_engine/control_models.py`",
        "`millrace_engine/control_reports.py`",
        "`millrace_engine/control_runtime_surface.py`",
        "`millrace_engine/cli_rendering.py`",
        "`millrace_engine/tui/gateway.py`",
        "`mailbox_task_intake` before backlog files visibly change",
        "`CompletionStateView`",
        "`status(detail=False)` is intentionally lighter than the richer surfaces.",
        "So the shell is not a pretty wrapper around `status --detail`.",
    ...
    "docs/runtime/recovery-failure-modes-timeouts-and-unwedge-playbook.md": (
        ...
        "`millrace_engine/engine_outage_recovery.py`",
        "`millrace_engine/control_actions.py`",
        "`millrace_engine/sentinel_runtime.py`",
        "`RUNNER_TIMEOUT after <seconds>s`",
        "`pause_resume`, `incident`, or `blocker`",
        "`usage_budget_threshold`",
        "`degraded_snapshot`",
    ...
    "docs/runtime/runtime-state-status-markers-and-stale-recovery-semantics.md": (
        ...
        "## 2. Source-Of-Truth Surfaces",
        "`millrace_engine/status.py`",
        "`millrace_engine/paths.py`",
        "`millrace_engine/control_models.py`",
        "`millrace_engine/control_reports.py`",
        "`millrace_engine/engine_runtime.py`",
        "`millrace_engine/control_actions.py`",
        "`agents/.runtime/state.json`",
        "`agents/.runtime/pending_active_task_clear.json`",
        "`recovery request <target> --issuer <name> --reason <text> --force-queue`",
