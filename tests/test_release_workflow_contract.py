...
def test_publish_workflow_smoke_checks_built_wheel_against_release_core_contract() -> None:
    ...
    expected_snippets = (
        ...
        '"spec_review": ("spec review complete", "IDLE")',
        ...
        'payload.get("status") == "TASKMASTER_RUNNING" and runtime.get("current_mode") == "GOALSPEC"',
        ...
        'assert research_start_payload["research_status"] == "OBJECTIVE_PROFILE_SYNC_RUNNING"',
        'assert research_report_payload["configured_mode"] == "auto"',
        'assert research_report_payload["status"] == "TASKMASTER_RUNNING"',
        'assert research_report_payload["runtime"]["last_mode"] == "STUB"',
        'assert research_report_payload["runtime"]["current_mode"] == "GOALSPEC"',
        'assert research_report_payload["runtime"]["checkpoint"]["node_id"] == "taskmaster"',
        'assert research_queue_payload["backlog_depth"] == 0',
        'assert research_supervisor_payload["attention_reason"] == "idle_with_pending_work"',
        'assert "Review-Status:" in research_spec_decision_text',
        'assert "Critic Findings" in research_spec_questions_text',
