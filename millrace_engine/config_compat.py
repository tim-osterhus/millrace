"""Compatibility helpers for deprecated markdown-based config surfaces."""
...
def load_model_values(path: Path) -> dict[str, str]:
    """Parse the active ``KEY=value`` block from a model-config markdown file."""
...
def load_workflow_values(path: Path) -> dict[str, str]:
    """Parse ``## KEY=value`` lines from a workflow-config markdown file."""
...
class LegacyPolicyCompatStatus(str, Enum):
    """Compatibility status for one markdown-era policy knob."""
...
class LegacyPolicyCompatCategory(str, Enum):
    """Audited compatibility categories."""
...
class LegacyPolicyCompatEntry(LegacyCompatModel):
    """Deterministic compatibility record for one config key."""
...
class LegacyPolicyCompatReport(LegacyCompatModel):
    """Audited compatibility surface for deprecated markdown policy keys."""
