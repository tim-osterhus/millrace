...
_CONFIG_FIELD_SPECS: tuple[_ConfigFieldSpec, ...] = (
    _ConfigFieldSpec(
...
_STARTUP_ONLY_FIELD_SPECS: tuple[_ConfigFieldSpec, ...] = (
    _ConfigFieldSpec(
        key="paths.workspace",
...
def _registry_ref(value: object | None) -> str | None:
    if value is None:
        return None
...
def _selection_ref(view: object | None) -> str | None:
    if view is None:
        return None
    return _registry_ref(getattr(view, "ref", None))


def _task_view(task: object | None) -> QueueTaskView | None:
    if task is None:
        return None
...
def _selection_summary(selection: object | None) -> SelectionSummaryView | None:
    if selection is None:
        return None
    return SelectionSummaryView(
        scope=str(getattr(selection, "scope")),
        selection_ref=_registry_ref(getattr(getattr(selection, "selection"), "ref")) or "unknown",
        mode_ref=_selection_ref(getattr(selection, "mode")),
        execution_loop_ref=_selection_ref(getattr(selection, "execution_loop")),
...
def _selection_decision(explanation: object | None) -> SelectionDecisionView | None:
...
def _stage_count(snapshot: object | None) -> int | None:
    if snapshot is None:
        return None
...
def _research_item_view(item: object | None) -> ResearchQueueItemView | None:
    if item is None:
        return None
...
class RuntimeGateway:
    ...
    def __init__(self, config_path: Path | str) -> None:
        ...
        self.config_path = resolved.resolve() if not resolved.is_absolute() else resolved

    def _new_control(self) -> EngineControl:
        return EngineControl(self.config_path)

    def _input_failure(self, operation: str, message: str) -> GatewayResult[Any]:
    ...
    def _failure(self, operation: str, exc: Exception) -> GatewayFailure:
    ...
    def _execute(self, operation: str, callback: Callable[[EngineControl], T]) -> GatewayResult[T]:
        try:
            return GatewayResult(value=callback(self._new_control()))
        except Exception as exc:  # noqa: BLE001
            return GatewayResult(failure=self._failure(operation, exc))

    def load_workspace_snapshot(self, *, log_limit: int = 50) -> GatewayResult[RefreshPayload]:
        if log_limit < 0:
            return self._input_failure("refresh.workspace", "log_limit must be non-negative")
        ...
        def callback(control: EngineControl) -> RefreshPayload:
            ...
            return RefreshPayload(
                ...
                runtime=self._runtime_overview(status),
                config=self._config_overview(config),
                queue=self._queue_overview(queue),
                research=self._research_overview(research, recent_activity=research_activity),
                events=EventLogView(
                    events=tuple(runtime_event_view(record) for record in events),
                    last_loaded_at=refreshed_at,
                ),
                runs=self._runs_overview(control, observed_at=refreshed_at),
            )

        return self._execute("refresh.workspace", callback)
    ...
    def load_publish_status(
        ...
        *,
        ...
        normalized_commit_message = self._normalized_optional_text(commit_message)
        ...
        def callback(control: EngineControl) -> RefreshPayload:
            ...
            return RefreshPayload(
                refreshed_at=_utcnow(),
                publish=self._publish_overview(report),
            )

        return self._execute("refresh.publish", callback)

    def load_run_detail(self, run_id: str) -> GatewayResult[RefreshPayload]:
        normalized_run_id = " ".join(run_id.strip().split())
        if not normalized_run_id:
            return self._input_failure("refresh.run_detail", "run_id is required")

        def callback(control: EngineControl) -> RefreshPayload:
            report = control.run_provenance(normalized_run_id)
            return RefreshPayload(
                refreshed_at=_utcnow(),
                run_detail=self._run_detail_view(report),
            )

        return self._execute("refresh.run_detail", callback)

    def add_task(self, title: str, *, body: str | None = None, spec_id: str | None = None) -> GatewayResult[ActionResultView]:
        normalized_title = " ".join(title.strip().split())
        if not normalized_title:
            return self._input_failure("action.add_task", "task title is required")
        normalized_body = self._normalized_optional_text(body)
        normalized_spec_id = self._normalized_optional_text(spec_id)
        return self._execute(
            "action.add_task",
            lambda control: self._operation_result(
                "add_task",
    ...
    def add_idea(self, source_file: Path | str) -> GatewayResult[ActionResultView]:
        ...
        return self._execute(
            "action.add_idea",
            lambda control: self._operation_result("add_idea", control.add_idea(source_path)),
        )

    def pause_runtime(self) -> GatewayResult[ActionResultView]:
        return self._execute("action.pause", lambda control: self._operation_result("pause", control.pause()))

    def resume_runtime(self) -> GatewayResult[ActionResultView]:
        return self._execute("action.resume", lambda control: self._operation_result("resume", control.resume()))

    def stop_runtime(self) -> GatewayResult[ActionResultView]:
        return self._execute("action.stop", lambda control: self._operation_result("stop", control.stop()))

    def reload_config(self) -> GatewayResult[ActionResultView]:
        return self._execute(
            "action.reload_config",
            lambda control: self._operation_result("reload_config", control.config_reload()),
        )

    def set_config(self, key: str, value: str) -> GatewayResult[ActionResultView]:
        normalized_key = " ".join(key.strip().split())
        if not normalized_key:
            return self._input_failure("action.set_config", "config key is required")
        return self._execute(
            "action.set_config",
            lambda control: self._operation_result("set_config", control.config_set(normalized_key, value)),
        )
    ...
    def reorder_queue(self, task_ids: list[str] | tuple[str, ...]) -> GatewayResult[ActionResultView]:
        ...
        if not normalized_ids:
            return self._input_failure("action.reorder_queue", "at least one task id is required")
        return self._execute(
            "action.reorder_queue",
            lambda control: self._operation_result("reorder_queue", control.queue_reorder(normalized_ids)),
        )

    def publish_sync(self, *, staging_repo_dir: Path | str | None = None) -> GatewayResult[ActionResultView]:
        return self._execute(
            "action.publish_sync",
            lambda control: self._sync_result(control.publish_sync(staging_repo_dir=staging_repo_dir)),
        )
    ...
    def publish_commit(
        ...
        *,
        ...
        return self._execute(
            ...
            lambda control: self._commit_result(
    ...
    def _runtime_overview(self, status: object) -> RuntimeOverviewView:
        ...
        return RuntimeOverviewView(
            ...
            selection=_selection_summary(getattr(status, "selection")),
            selection_decision=_selection_decision(getattr(status, "selection_explanation")),
        )

    def _config_overview(self, report: object) -> ConfigOverviewView:
        ...
        def build_field(spec: _ConfigFieldSpec, *, editable: bool) -> ConfigFieldView:
            boundary = getattr(boundaries, "classify_field")(spec.key)
            return ConfigFieldView(
                key=spec.key,
                label=spec.label,
                value=stringify_value(_config_value(config, spec.key)) or "none",
        ...
        return ConfigOverviewView(
            ...
            fields=tuple(build_field(spec, editable=editing_enabled) for spec in _CONFIG_FIELD_SPECS),
            startup_only_fields=tuple(build_field(spec, editable=False) for spec in _STARTUP_ONLY_FIELD_SPECS),
        )

    def _queue_overview(self, queue: object) -> QueueOverviewView:
        return QueueOverviewView(
            active_task=_task_view(getattr(queue, "active_task")),
            next_task=_task_view(getattr(queue, "next_task")),
            backlog_depth=int(getattr(queue, "backlog_depth")),
            backlog=tuple(_task_view(task) for task in getattr(queue, "backlog") if _task_view(task) is not None),
        )
        ...
        *,
        ...
        queue_families = tuple(
            ResearchQueueFamilyView(
                ...
                first_item=_research_item_view(getattr(family, "first_item")),
            )
        ...
        return ResearchOverviewView(
            ...
            audit_summary=self._research_audit_summary(report),
            governance=self._research_governance(report),
            recent_activity=tuple(runtime_event_view(record) for record in recent_activity),
        )

    def _research_audit_summary(self, report: object) -> ResearchAuditSummaryView | None:
    ...
    def _research_governance(self, report: object) -> ResearchGovernanceOverviewView | None:
        ...
        if self._governance_has_signal(view):
            return view
        return None

    @staticmethod
    def _governance_has_signal(view: ResearchGovernanceOverviewView) -> bool:
        return any(
            (
    ...
    def _publish_overview(self, report: PublishPreflightReport) -> PublishOverviewView:
    ...
    def _runs_overview(self, control: EngineControl, *, observed_at: datetime) -> RunsOverviewView:
        ...
        if runs_dir.exists():
            ...
            for run_dir in sorted(run_dirs, key=self._run_sort_key, reverse=True)[:RECENT_RUN_LIMIT]:
                runs.append(self._run_summary(run_dir))
    ...
    def _run_summary(self, run_dir: Path) -> RunSummaryView:
        ...
        return RunSummaryView(
            ...
            selection_ref=(
                _registry_ref(getattr(compile_snapshot, "selection_ref", None))
                if compile_snapshot is not None
                else _registry_ref(getattr(frozen_plan, "selection_ref", None))
            ),
            ...
            stage_count=_stage_count(compile_snapshot),
    ...
    def _policy_evidence_view(self, evidence: object | None) -> RunPolicyEvidenceView | None:
        if evidence is None:
            return None
    ...
    def _integration_policy_view(self, integration: object | None) -> RunIntegrationSummaryView | None:
    ...
    def _run_detail_view(self, report: object) -> RunDetailView:
        ...
        return RunDetailView(
            ...
            frozen_plan_hash=(getattr(frozen_plan, "content_hash", None) if frozen_plan is not None else None),
            stage_count=_stage_count(compile_snapshot),
            selection=_selection_summary(getattr(report, "selection")),
            selection_decision=_selection_decision(getattr(report, "selection_explanation")),
            current_preview=_selection_summary(getattr(report, "current_preview")),
            current_preview_decision=_selection_decision(getattr(report, "current_preview_explanation")),
            ...
            latest_policy_evidence=self._policy_evidence_view(getattr(report, "latest_policy_evidence")),
            integration_policy=self._integration_policy_view(getattr(report, "integration_policy")),
            transitions=transitions,
        )

    def _operation_result(self, action: str, result: object) -> ActionResultView:
    ...
    def _sync_result(self, report: StagingSyncReport) -> ActionResultView:
        return ActionResultView(
            action="publish_sync",
    ...
    def _commit_result(self, report: PublishCommitReport) -> ActionResultView:
