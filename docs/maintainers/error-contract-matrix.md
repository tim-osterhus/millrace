...
| Queue closure | `adapters/cli/queue.py`, `kernel/decision.py`, `kernel/mutations.py`, `substrate/_sqlite_*` | JSON `code` / transition reason | Success: `queued_work_cancelled`, `queued_lineage_cancelled`; refusal: `queued_work_not_found`, `queued_lineage_not_found`, `queued_work_not_cancelable`, `queued_work_plan_mismatch`, `queued_lineage_plan_mismatch`, `queued_work_claimed`, `queued_runner_session_live`, `queued_runner_session_lost`, `queued_runner_session_missing`, `queued_runner_cleanup_unresolved`, `selected_plan_mismatch`, `idempotency_conflict` | Public stable | Re-query status and runner aftermath. Use `runs cancel` for live runner control; use a new input ID only for a materially new closure request. | CLI forms audited operator input; kernel derives and fences the complete close set; substrate persists queue-closure audit and existing close-work records atomically. Compiler parity: N/A for operator/current-state conditions. | `tests/cli/test_cli_queue_commands.py::test_queue_cancel_closes_one_queued_work_item_and_replays`; `tests/kernel/test_queue_closure.py::test_queue_lineage_closure_preflights_and_closes_complete_membership`; `tests/substrate/test_persistence_integrity_refusals.py::test_corrupt_queue_closure_row_refuses_without_mutation` | [Command Codes](#command-codes) |
...
| Dispatch suspend/resume | `adapters/cli/dispatch.py`, `kernel/decision.py`, `kernel/mutations.py`, `substrate/_sqlite_*` | JSON `code` / transition reason | Success: `dispatch_suspended`, `dispatch_resumed`; refusal: `dispatch_suspended`, `dispatch_already_suspended`, `dispatch_not_suspended`, `dispatch_suspension_identity_mismatch`, `selected_plan_mismatch`, `stale_dispatch_generation`, `idempotency_conflict` | Public stable | Re-query status, preserve the exact suspension identity and plan fingerprint, and use a new input ID only for a materially new command. | CLI forms audited input; kernel owns admission/fencing; substrate owns the versioned record and atomic persistence. Compiler parity: N/A for operator/current-state conditions. | `tests/cli/test_cli_dispatch_commands.py::test_dispatch_suspend_and_exact_resume_are_replay_safe`; `::test_dispatch_resume_refuses_wrong_suspension_identity`; `tests/kernel/test_enqueue_and_claim.py::test_claim_and_suspend_decisions_are_mutually_fenced` | [Dispatch And Readiness](#dispatch-and-readiness) |
...
| Daemon budget and usage | `adapters/cli/daemon.py`, `adapters/cli/session_completion.py`, `substrate/sqlite.py` | CLI/bounded-unit `code`; durable refusal reason | `budget_id_required`, `budget_limit_required`, `runner_usage_mapping_unsupported`, `budget_plan_pin_refused`, `daemon_budget_epoch_refused`, `daemon_budget_limit_out_of_range`, `daemon_budget_immutable_limits_changed`, `daemon_budget_clock_discontinuity`, `runner_usage_evidence_refused` | Public stable in the named fields. Immutable-limit and clock discontinuity reasons surface as their exact CLI domain-refusal codes. Only unrecognized durable epoch failures map to `daemon_budget_epoch_refused`. | Correct intake, selected-plan pin, clock continuity, or reviewed usage mapping. Preserve the existing epoch and use a new budget ID for materially different limits. | Intake refusals create no epoch. Immutable-limit and clock refusals preserve the existing active epoch. Accepted usage mutates one fenced session and its epoch atomically. Governed usage refusal alone terminalizes the epoch and suspends only new dispatch; it does not create workflow meaning or billing truth. | `tests/cli/test_cli_daemon_loop.py::test_daemon_budget_options_refuse_invalid_limits_before_state_open`; `::test_token_budget_adapter_admission_precedes_all_runtime_work`; `::test_daemon_budget_epoch_maps_wall_deadline_overflow_to_stable_refusal`; `::test_daemon_budget_epoch_maps_corrupt_durable_load_to_stable_refusal`; `::test_pending_budget_reservation_refuses_foreign_ownership_without_rewrite`; `::test_daemon_budget_epoch_restart_clock_boundaries_preserve_authority`; `::test_governed_usage_terminal_path_matrix` | [Daemon Budget Invariant Matrix](daemon-budget-invariant-matrix.md); [Dispatch Suspension Invariant Matrix](dispatch-suspension-invariant-matrix.md); [Queue Closure Invariant Matrix](queue-closure-invariant-matrix.md) |
...
- Queue: `invalid_payload_source`, `payload_file_unreadable`,
  `queued_work_not_found`, `queued_lineage_not_found`,
  `queued_work_not_cancelable`, `queued_work_plan_mismatch`,
  `queued_lineage_plan_mismatch`, `queued_work_claimed`,
  `queued_runner_session_live`, `queued_runner_session_lost`,
  `queued_runner_session_missing`,
  `queued_runner_cleanup_unresolved`, plus the operator-input codes below.
...
## Runner-Session Error Matrix

| Category | Owner | Stable field/codes | Mutation behavior | Operator action | Test proof |
| --- | --- | --- | --- | --- | --- |
| Kernel session refusal | `kernel/runner_sessions.py`, `kernel/decision.py`, `kernel/mutations.py` | refusal `reason`: `invalid_runner_session_transition`, `stale_runner_session`, `duplicate_runner_session_completion`, `runner_session_authority_mismatch`, `runner_session_retry_forbidden`, `runner_session_cleanup_incomplete`, `runner_session_reconciliation_contradiction` | Refusal cannot create workflow meaning, replace the current session, or repair state; audited attempts use the existing refusal/event/trace path. | Inspect current run/session identity, completion, cleanup, and selected authority before retrying. | `tests/kernel/test_runner_session_lifecycle.py`; `tests/adapters/test_session_coordinator.py` |
| Session CLI | `adapters/cli/status.py`, `adapters/cli/session_coordinator.py` | CLI `code`: `runner_session_cancel_requested`, `runner_session_cancel_refused`, `runner_session_retry_refused`, `runner_session_orphan_risk` | Accepted cancellation persists one replay-safe request; refused cancel/retry does not perform the requested session mutation; orphan risk is a durable safety stop. | Use `runs show`, `trace show`, `status`, and `doctor`; do not replace lost/orphan-risk work. | `tests/cli/test_cli_daemon_session.py::test_runs_cancel_persists_fixed_operator_reason_and_replays`; `::test_runs_cancel_refuses_terminal_session`; `tests/adapters/test_session_coordinator.py::test_orphan_risk_explicit_retry_preserves_claim_and_refuses_replacement` |
| Store schema compatibility | `adapters/cli/context.py`, `adapters/cli/main.py`, `substrate/sqlite.py` | CLI `code`: `workspace_upgrade_required` | Exact schema 6 and schema 7 are refused before initialization or execution; database, CAS, and event sidecar remain byte-for-byte unchanged. No automatic migration exists. Exact schema 8, 9 or 10 is an unsupported-schema refusal, also without automatic migration. | Finish or retire active work with its matching runtime, then initialize a fresh schema-11 workspace. | `tests/cli/test_cli_workspace_commands.py::test_workspace_check_maps_prior_schema_to_upgrade_required_json_and_human`; `::test_daemon_maps_prior_schema_to_upgrade_required_without_mutation`; `tests/substrate/test_sqlite_schema.py::test_schema_8_open_and_initialize_refuse_without_byte_mutation`; `tests/substrate/test_sqlite_schema.py::test_initialize_refuses_store_schema_version_9_without_mutation` |
| Session doctor | `adapters/cli/doctor.py` | diagnostic `code`: `runner_session_lost`, `runner_session_orphan_risk`, `runner_session_reconciliation_unsupported`, `runner_session_cleanup_pending` | Read-only bounded projection; no reconciliation or repair. | Preserve the claim/session, inspect durable evidence, and resolve externally before any reviewed intervention. | `tests/cli/test_cli_daemon_session.py::test_daemon_reconciles_restart_sessions_before_new_dispatch`; `tests/cli/test_cli_doctor_commands.py::test_doctor_is_minimal_generic_read_only_projection` |
| Adapter outcome | `adapters/runner_contract.py`, selected adapter | adapter `error_kind`: `timeout`, `cancelled`, `invocation_failed`, `result_parse_failed`, `input_too_large`, `output_too_large`, `redaction_refused`, `selected_authority_refused` | Candidate-evidence failure only; it cannot route or close workflow work. Truncation is metadata unless normalization fails. | Correct local adapter/config/output bounds or reconcile the durable session; do not translate the error into workflow meaning. | `tests/adapters/test_runner_contract.py::test_each_adapter_error_kind_cannot_convert_to_evidence`; `tests/adapters/test_session_coordinator.py` |
