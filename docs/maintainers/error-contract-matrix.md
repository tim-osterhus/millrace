...
| Schema-6/7 compatibility | `adapters/cli/context.py`, `adapters/cli/main.py`, `substrate/sqlite.py` | CLI `code`: `workspace_upgrade_required` | Exact schema 6 and schema 7 are refused before initialization or execution; database, CAS, and event sidecar remain byte-for-byte unchanged. No automatic migration exists. | Finish or retire active work with its matching runtime, then initialize a schema-8 workspace. | `tests/cli/test_cli_workspace_commands.py::test_workspace_check_maps_prior_schema_to_upgrade_required_json_and_human`; `::test_daemon_maps_prior_schema_to_upgrade_required_without_mutation` |
