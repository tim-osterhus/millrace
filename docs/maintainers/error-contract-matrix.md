...
| Schema-6 compatibility | `adapters/cli/context.py`, `adapters/cli/main.py`, `substrate/sqlite.py` | CLI `code`: `workspace_upgrade_required` | Exact schema 6 is refused before initialization or execution; database, CAS, and event sidecar remain byte-for-byte unchanged. No automatic migration exists. | Finish or retire active work with the v0.22 runtime, then initialize a schema-7 workspace. | `tests/cli/test_cli_workspace_commands.py::test_workspace_check_maps_exact_v6_to_upgrade_required_json_and_human`; `::test_daemon_maps_exact_v6_to_upgrade_required_without_mutation` |
