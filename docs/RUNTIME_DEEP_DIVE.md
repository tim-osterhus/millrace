...
This file remains the stable runtime deep-dive portal path. The modular boundary docs now live under `docs/runtime/`; start with `docs/runtime/README.md` for the catalog, `docs/runtime/runtime-loop-lifecycle-and-supervisor-authority.md` for lifecycle ownership, once/daemon sequencing, and liveness authority, `docs/runtime/control-plane-command-surface-and-mailbox-semantics.md` for immediate versus mailbox-deferred control mutations and pending-command visibility, `docs/runtime/runtime-state-status-markers-and-stale-recovery-semantics.md` for persisted runtime state, status-marker contracts, stale snapshot normalization, and supported recovery controls, `docs/runtime/stage-execution-pipeline-and-plane-handoff-contracts.md` for stage legality, transition-history recording, and execution-to-research handoff composition, `docs/runtime/runner-adapters-model-selection-and-permission-profile-semantics.md` for stage config defaults, Codex/Claude/Subprocess command construction, and the exact `normal`/`elevated`/`maximum` permission-profile contract, `docs/runtime/configuration-surfaces-apply-boundaries-and-live-reload-semantics.md` for native TOML ownership, runtime apply-boundary timing, queued pending config, and startup-only rejection rules, `docs/runtime/observability-reports-tui-and-audit-truth-surfaces.md` for the report-builder hierarchy, audit/completion visibility, CLI rendering, and TUI snapshot narrowing contract, and `docs/runtime/recovery-failure-modes-timeouts-and-unwedge-playbook.md` for bounded failure signatures, timeout evidence, outage routing, Sentinel escalation states, and supported operator recovery steps.
...
For the bounded observability contract behind status, supervisor, research, run-provenance, and shell rendering, see `docs/runtime/observability-reports-tui-and-audit-truth-surfaces.md`.
