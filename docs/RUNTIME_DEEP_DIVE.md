...
This file remains the stable runtime deep-dive portal path. The modular boundary docs now live under `docs/runtime/`; start with `docs/runtime/README.md` for the catalog, `docs/runtime/runtime-loop-lifecycle-and-supervisor-authority.md` for lifecycle ownership, once/daemon sequencing, and liveness authority, and `docs/runtime/control-plane-command-surface-and-mailbox-semantics.md` for immediate versus mailbox-deferred control mutations and pending-command visibility.
