...
| --- | --- | --- |
| MR-MAINT-001 | Complete | FU-2 moved Blueprint runtime-effect behavior into `extensions/builtin/blueprint/operation_runners/`; the runtime-facing compatibility export was retired from `runtime/effects/operation_runners/__init__.py`. |
...
- MR-MAINT-001 through MR-MAINT-010 are complete for this follow-up wave. New
  ...
  `runtime/completion_behavior.py`, and `extensions/builtin/blueprint/context.py`)
  rather than re-opening the finished packet ids.
...
| Built-in family branching | `src/millrace_ai/runtime/completion_behavior.py` | Runtime kernel compatibility boundary | Extension packages | Work-family branch manifests | **Named boundary in place; two hard-coded `WorkItemKind` branches remain as compatibility-only.** The `closure_boundary.py` module is the kernel-facing boundary; `completion_behavior.py` is the internal implementation behind it. Two `WorkItemKind` comparisons remain:
  - `WorkItemKind.SPEC` at line 122 (`prepare_closure_target_for_claim`): spec documents are loaded for closure-target preparation; non-spec claims bypass spec loading. Allowed callers: closure-boundary internal implementation only (routed through `closure_boundary.py`). Mode scope: all modes; lazy delegation ensures the closure implementation loads only when closure behavior is needed. Retention reason: spec document loading is closure-target lifecycle logic not yet migrated to operation-step primitives. Guardrail proof: `test_kernel_code_does_not_add_new_direct_domain_imports` allowlists `completion_behavior` as a compatibility facade; subprocess import/startup probes prove zero forbidden prefixes on public import.
  - `WorkItemKind.BLUEPRINT_DRAFT.value` at line 229 (`_closure_blocking_refs_from_inventory`): Blueprint draft work refs are excluded from closure-blocking lineage checks. Allowed callers: closure-boundary internal implementation only. Mode scope: Blueprint-declaring modes. Retention reason: Blueprint draft exclusion is closure-lineage blocking policy not yet migrated to work-family adapter or compiled policy metadata. Guardrail proof: `test_no_work_item_kind_enum_branch_dispatch_in_generic_runtime_paths` allowlists `completion_behavior.py` for WorkItemKind enum dispatch. |
