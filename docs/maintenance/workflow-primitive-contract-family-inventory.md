...
Status: scaffolded package baseline for Batch 7 Packet 01
Source candidate: `MR-MAINT-004`
Current source: `src/millrace_ai/architecture/workflow_primitives/__init__.py`

This inventory records the current public contract families before the full
contract-family split. The package scaffold is in place and shared helpers now
live in `workflow_primitives/identifiers.py` and `workflow_primitives/_validation.py`.
