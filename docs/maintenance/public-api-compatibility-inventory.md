...
| `src/millrace_ai/runtime/effects/operation_runners/__init__.py` | Generic runtime-effect helper package initializer | Empty export surface; shipped Blueprint operations live in `extensions/builtin/blueprint/operation_runners/` and generic helper modules remain under `runtime/effects/operation_runners/` | Runtime-effect registry and generic-kernel guardrails ensure the package initializer stays empty and generic paths do not select Blueprint branches |
