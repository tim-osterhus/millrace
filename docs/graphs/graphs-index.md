...
- `execution-lad.md`: LAD Execution plane for task implementation,
  verification, repair, update, troubleshooting, and planning escalation.
- `execution-lad-integrator.md`: high-assurance Execution plane that inserts
  Integrator between Builder and Checker.
- `planning-lad.md`: LAD Planning plane for Recon, Planner, Manager,
...
| --- | --- | --- | --- |
| `lad_codex` | Codex | `execution.lad`, `planning.lad` | Baseline Codex-backed LAD code-development runtime. It runs LAD Execution and Planning without Learning or Integrator. |
| `lad_pi` | Pi | `execution.lad`, `planning.lad` | Pi-backed equivalent of `lad_codex`. The graph topology is identical; only the stage runner bindings change. |
| `learning_lad_codex` | Codex | `execution.lad`, `planning.lad`, `learning.standard` | Adds the Learning plane to the Codex baseline. Learning may run concurrently with one foreground Execution or Planning stage and receives Analyst/Librarian trigger work. |
| `efficient_learning_lad_mixed` | Mixed Codex/Pi | `execution.lad`, `planning.lad`, `learning.standard` | Uses the same topology as `learning_lad_codex`, keeps Integrator off by default, and ships a mode-local alias profile that assigns Codex stages and Pi/DeepSeek stages explicitly. |
| `learning_lad_pi` | Pi | `execution.lad`, `planning.lad`, `learning.standard` | Pi-backed equivalent of `learning_lad_codex`. The selected plane graphs and learning trigger rules are identical; the mode binds stages to `pi_rpc`. |
| `lad_codex_integrated` | Codex | `execution.lad_integrator`, `planning.lad` | Uses the high-assurance LAD Execution graph with Integrator after Builder. Planning remains LAD and Learning is not selected. |
| `learning_lad_codex_integrated` | Codex | `execution.lad_integrator`, `planning.lad`, `learning.standard` | Combines Integrator-backed LAD Execution with the standard Learning plane. Learning trigger rules and concurrency policy match other learning-enabled modes. |
| `blueprint_lad_codex` | Codex | `execution.lad`, `planning.blueprint` | Uses LAD Execution plus Blueprint Planning. Planner output is decomposed into Blueprint drafts and approved generated tasks before Execution claims them. |
| `blueprint_learning_lad_codex` | Codex | `execution.lad`, `planning.blueprint`, `learning.standard` | Adds Learning to the Blueprint mode. Planner completion still triggers Librarian while Blueprint Planning continues toward Manager Blueprint and draft-packet work. |
...
The current Pi shipped modes cover the LAD and LAD plus Learning
